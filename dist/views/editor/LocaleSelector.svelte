...
<style>
	...
	button.label svg#chevron {
			transition: 0.1s transform;
		}

	button.label:hover:not(.active) svg#chevron {
			transform: translateY(2px);
		}

	button.label.active svg#chevron {
			transform: scaleY(-1);
		}
	...
	.select-container .language-list {
			display: grid;
			overflow-y: auto;
			max-height: 20rem;
		}

	.select-container .language-list .language-item {
				display: flex;
			}

	.select-container .language-list button:hover,
				.select-container .language-list button:focus,
				.select-container .language-list button.active {
					background: #1f1f1f;
				}
	...
	.select-container .language-list button.option {
				flex: 1;
				text-align: left;
				font-weight: 400;
				white-space: nowrap;
	...
	.select-container .language-list button.remove {
				padding: 0 0.5rem;
			}
