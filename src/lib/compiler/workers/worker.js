import { rollup } from '../lib/rollup-browser.js'
	...
	function generate_lookup(component) {
		if (Array.isArray(component)) {
			...
			component_lookup.set(
              ...
              <script>
              ${component
								.map((_, i) => `import Component_${i} from './Component_${i}.svelte';`)
								.join('')}
              </script>
              ${component
								.map((section, i) => {
			...
			plugins: [
				{
					...
					async resolveId(importee, importer) {
						...
						return `https://cdn.skypack.dev/${importee}` // use skypack as a fallback
						...
						if (/.*\.svelte/.test(id)) {
							try {
								...
								const warnings = res.warnings
									...
									.filter(
										(w) => !w.message.startsWith(`A11y: <img> element should have an alt attribute`)
									)
